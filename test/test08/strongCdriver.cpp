...
int main(int argc, char *argv[]) {  //fake train alg
  ...
  char good [] = "../../dataset/sample/good/";
  char bad [] = "../../dataset/sample/bad/";
 ...
 printf("sum is %f\n",sum);
  ...
  for(int k = 0; k< WEAK;k++){
    ...
    for(int j = 0; j< m+l;j++){
      ...
      printf("FVALUE for k %d is %d \n",k,weak_grid[k].featurevalue(image_list[j]));
    }
   ...
   for(auto w:weak_grid){
	...
	printf("check:%d,%f\n",j,errorsum);
      ...
      float adder = weights[i]*abs(weak_grid[j].classify(image_list[i]) - y);
      
      printf("image is %d, weakc is %d,a is %d,weights:%f,classified as %d\n",i,j,a,weights[i],weak_grid[j].classify(image_list[i]));
     printf("sum is %f, adder is %f\n",errorsum,adder);
      ...
      if(abs(weak_grid[j].classify(image_list[i]) - y)){
         notequal += 1;
      }
 ...
 //cout << "Remove weakc:"<< mark <<" from the grid. Old weak_grid size is "<< weak_grid.size()<< " Old weakc_final size is "<<weakc_final.size()<<'\n';
  ...
  alpha.push_back(log(1/b));
   ...
   printf("weight at i is %f",weights[i]);
   printf("p is %f\n",p);
