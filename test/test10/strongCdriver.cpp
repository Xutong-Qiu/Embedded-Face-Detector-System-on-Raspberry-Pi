...
int main(int argc, char *argv[]) {  //fake train alg
  ...
  char good [] = "../../dataset/good_integral/validation/";
  char bad [] = "../../dataset/bad_integral/validation/";
 ...
 printf("sum is %f\n",sum);
  ...
  for(int k = 0; k< featuresize-a;k++){
    ...
    for(int j = 0; j< m+l;j++){
      fvalue.push_back(weak_grid[k].featurevalue(image_list[j]));    //just return the difference
      //printf("%d ",weak_grid[k].featurevalue(image_list[j]));
    }
